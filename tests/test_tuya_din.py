    ...
    for call in expected_calls:
        ...
        if method_name == "energy_deliver_reported":
            ...
            cluster.endpoint.smartenergy_metering.energy_deliver_reported.assert_called_with(value)
                           ...
                           "power_factor_reported", "ac_frequency_reported"]:
...
async def test_hiking_power_factor(hiking_cluster):
    ...
    hiking_cluster.endpoint.electrical_measurement.power_factor_reported.assert_called_with(950)
...
async def test_zemismart_energy_reporting(zemismart_cluster):
    ...
    zemismart_cluster.endpoint.device.clamp_bus["energy"]["abc"].listener_event.assert_called_with(
        "energy_reported", 100
    )
    ...
    for dp_id, phase in [(0x0215, "a"), (0x0216, "b"), (0x0217, "c")]:
        ...
        zemismart_cluster.endpoint.device.clamp_bus["energy"][phase].listener_event.assert_called_with(
            "energy_reported", 50
        )
